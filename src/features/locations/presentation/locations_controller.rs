...
pub async fn update_location(
    ...
    validate_location_body(&body)?;
    if LocationRepository::find_by_id(&state, &ctx.paths, &id)
        .await?
        .is_none()
    {
    ...
    let loc = LocationRepository::update(&state, &ctx.paths, &id, payload).await?;
    Ok(SuccessResponse::data(location_to_json(&loc)))
}
...
pub async fn delete_location(
    ...
    LocationRepository::delete(&state, &ctx.paths, &id).await?;
