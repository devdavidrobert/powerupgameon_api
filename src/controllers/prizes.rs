...
pub async fn get_prize(
    ...
    let prize = PrizeModel::find_by_id(&state, &id)
        .await?
...
pub async fn create_prize(
    ...
    let all = PrizeModel::find_all(&state).await?;
    ...
    let prize = PrizeModel::create(&state, data).await?;
...
pub async fn update_prize(
    ...
    if PrizeModel::find_by_id(&state, &id).await?.is_none() {
    ...
    let updated = PrizeModel::update(&state, &id, updates).await?;
...
pub async fn delete_prize(
    ...
    PrizeModel::delete(&state, &id).await?;
