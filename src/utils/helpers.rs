...
pub fn submission_identity_from_registration(
    reg: &Map<String, Value>,
    ...
    let full_name = reg
        ...
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .ok_or_else(|| ApiError::bad_request("Registration is missing a valid fullName."))?;
    let normalized_name = reg
        .get("normalizedName")
        .and_then(|v| v.as_str())
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .ok_or_else(|| ApiError::bad_request("Registration is missing a valid normalizedName."))?;
